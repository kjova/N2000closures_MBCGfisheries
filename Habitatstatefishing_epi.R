...
FisheriesMet <- cbind(VMSdatMet, CsquareInfo[match(VMSdatMet$csquares,CsquareInfo$csquares), c("epi_intercept","epi_slope")])
FisheriesMet <- subset(FisheriesMet, is.na(epi_intercept)==FALSE) # remove all csquares with missing epi_intercept 
FisheriesMet <- subset(FisheriesMet, is.na(epi_slope)==FALSE) # remove all csquares with missing epi_slope

# depletion rates is based on  Hiddink et al. PNAS 2017 / Rijnsdorp et al. ICES 2020
for (i in 1: length(Period)){
  loopdata <- as.data.frame(FisheriesMet)
  ...
  # calculate epi_slope and epi_intercept and use RBS function
  state <- rep(1,nrow(loopdata))
  
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_tot[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_tot[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_DRB_MOL[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_DRB_MOL[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_CRU[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_CRU[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_DMF[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_MIX[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_MIX[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_SPF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_SPF[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_SDN_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_SDN_DMF[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_SSC_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_SSC_DMF[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_CRU[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_CRU[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_DMF[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_MOL[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_MOL[j],a=loopdata$epi_slope[j],b=loopdata$epi_intercept[j],H=H)
    }}
...
State_reg <- data.frame(FisheriesMet$csquares,state_year)
