...
FisheriesMet <- cbind(VMSdatMet, CsquareInfo[match(VMSdatMet$csquares,CsquareInfo$csquares), c("inf_intercept","inf_slope")])
FisheriesMet <- subset(FisheriesMet, is.na(inf_intercept)==FALSE) # remove all csquares with missing intercept 
FisheriesMet <- subset(FisheriesMet, is.na(inf_slope)==FALSE) # remove all csquares with missing slope
...
for (i in 1: length(Period)){
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_tot[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_tot[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_DRB_MOL[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_DRB_MOL[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_CRU[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_CRU[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_DMF[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_MIX[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_MIX[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_OT_SPF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_OT_SPF[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_SDN_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_SDN_DMF[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_SSC_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_SSC_DMF[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_CRU[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_CRU[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_DMF[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_DMF[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
  ...
  for(j in 1:nrow(loopdata)){
    if (loopdata$Depl_TBB_MOL[j] > 0 ) {
      state[j] <- RBS(Fd=loopdata$Depl_TBB_MOL[j],a=loopdata$inf_slope[j],b=loopdata$inf_intercept[j],H=H)
    }}
